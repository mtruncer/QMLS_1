...
Type `str(SSD)` into the code chunk below and then compile or execute the code chunk in the R console.
...
First change `eval: false` to `eval: true`. Changing this flag in the chunk will cause RStudio to evaluate the chunk when it is rendering the document. When `eval` is `false`, the chunk will be skipped. This is something that we need to do when writing the problem sets, so that they will compile.
...
To give you some practice filtering, filter your data in the following ways using a) base R functions and b) tidyverse functions. Verify your results match in each case. You do not need to save the resulting `data.frame`s.
