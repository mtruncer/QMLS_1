...
- ANOVA is robust to minor violations of these assumptions
- Equal sample sizes helps with violations of equal variances
- ANOVA is the *maximum likelihood estimate* given these assumptions
...
Grand_mean <- mean(JL$Shift)
Control <- JL |> 
  filter(Treatment == "control") |> 
  mutate(Resid_total = Shift - Grand_mean,
         Resid_group = mean(Shift) - Grand_mean,
         Resid_error = Shift - mean(Shift),
         Treatment = "control")
Knee <- JL |> 
  filter(Treatment == "knee") |> 
  mutate(Resid_total = Shift - Grand_mean,
         Resid_group = mean(Shift) - Grand_mean,
         Resid_error = Shift - mean(Shift),
         Treatment = "knee")
Eyes <- JL |> 
  ...
  mutate(Resid_total = Shift - Grand_mean,
         ...
         Resid_error = Shift - mean(Shift),
         Treatment = "eyes")
JL_summary <- bind_rows(Control, Knee, Eyes)

ggplot() +
  geom_segment(data = JL_summary,
               aes(x = Treatment))
  geom_point(data = JL_summary,
             aes(x = Treatment, y = Shift, color = Treatment),
             position = position_jitter(width = 0.2, seed = 1121))
```
