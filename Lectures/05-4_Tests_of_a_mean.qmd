...
Vector of likelihoods for all values in `temps` given `mu = 98.6` and `sigma = 0.1`:
...
Define a function that takes a vector of values to optimize `theta` ($\mu$ and $\sigma$) as well as a set of data `Y` and returns the log-likelihood:
