...
> I'm not sure if we can go over this on Friday, but I'd really like some clarification on one of the PC_01 questions - specifically the one about joining the EVI.csv data set with the modified soil respiration rates data set. The problem says that "that the EVI dataset does not currently have unique indicators (or key values) to merge with the soil respiration data", but both sets have Rs values. Why couldn't you modify the Rs outlier on the EVI set and then join them on Rs values? What would be the advantage to creating unique IDs?
