...
grid_approx <- crossing(mu, sigma) # <1>
```
...
grid_approx[which.max(grid_approx$log_lik), ] # <1>
```
...
- Grid approximation is slow and imprecise
...
optim(par = c(99, 0.5), # Start at 100, 0.1
      fn = log_lik,
      Y = temps,
...
1. `fnscale = -1` tells `optim()` to minimize the function
